...
## הרשאות
שלוש רמות, כל אחת עם סיסמה נפרדת (או בלי סיסמה בשביל צפייה):
- **צופה**: כל מי שיש לו את הקישור, בלי סיסמה. רואה הכל, לא משנה כלום.
- **מפעיל** (`OPERATOR_PASSWORD`): בנוסף לצפייה, יכול לרשום הפקדות (בודדות ומרוכזות) ולרשום הגרלות חדשות. לא יכול לגעת בבני משפחה (הוספה/פרישה), לא בגיבויים, ולא באיפוס כללי. מיועד לעוזרים לא-טכניים כדי שלא יסכנו את המערכת בטעות.
- **מנהל** (`ADMIN_PASSWORD`): הכל, כולל ניהול חברים, גיבויים ואיפוס.

כדי "להוסיף" מנהל/מפעיל נוסף – פשוט משתפים את הסיסמה המתאימה עם עוד בן משפחה (זו סיסמה משותפת לתפקיד, לא חשבון אישי). הטוקן נשמר בדפדפן שלו (localStorage), תקף ל-12 שעות ואז צריך להתחבר שוב.

אין למחוק חבר במובן האמיתי – יש כפתור "פרש" (soft delete, מנהל בלבד) ששומר את ההיסטוריה שלו ומוציא אותו מהגרלות עתידיות. אפשר להחזיר.
